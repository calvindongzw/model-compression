...
class STE(torch.autograd.Function):
    @staticmethod
    def forward(ctx, w, bit):  # "w" is the weight tensor (as a PyTorch Tensor), "bit" is the precision to be quantized to.
        if bit is None:
            wq = w         # Full precision, no change
        elif bit==0:
            wq = w*0       # Zero precision, everything is zero 
        else:
            alpha = w.max() - w.min()       # Compute alpha (scale) for dynamic scaling
            beta = w.min()     # Compute beta (bias) for dynamic scaling
